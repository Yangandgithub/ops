...
class ModelResultsCollectorToSave(CallbackBase):
    ...
    def v2_runner_on_unreachable(self, result):
        ...
        data = "{host} | UNREACHABLE! => {stdout}".format(host=result._host.get_name().decode('utf-8'),
                                                          stdout=json.dumps(result._result, indent=4))
        DsRedis.OpsAnsibleModel.lpush(self.redisKey, data)
        if self.logId: AnsibleSaveResult.Model.insert(self.logId, data, result._host.get_name().decode('utf-8'), 1)
    ...
    def v2_runner_on_ok(self, result, *args, **kwargs):
        ...
        if result._result.has_key('rc') and result._result.has_key('stdout'):
            if result._result.get('stdout').strip() == '' and result._result.has_key('stderr'):
                data = "{host} | SUCCESS | rc={rc} >> \n{stdout}".format(host=result._host.get_name().decode('utf-8'),
                                                                         rc=result._result.get('rc'),
                                                                         stdout=result._result.get('stderr'))
                data1 = "{stdout}".format(stdout=result._result.get('stderr'))
                data2 = "{host}|{stdout}".format(host=result._host.get_name().decode('utf-8'),
                                                 stdout=result._result.get('stderr'))
            else:
                data = "{host} | SUCCESS | rc={rc} >> \n{stdout}".format(host=result._host.get_name().decode('utf-8'),
                                                                         rc=result._result.get('rc'),
                                                                         stdout=result._result.get('stdout'))
                data1 = "{stdout}".format(stdout=result._result.get('stdout'))
                data2 = "{host}|{stdout}".format(host=result._host.get_name().decode('utf-8'),
                                                 stdout=result._result.get('stdout'))
            ...
            data = "{host} | SUCCESS >> {stdout}".format(host=result._host.get_name().decode('utf-8'),
                                                         stdout=json.dumps(result._result, indent=4))
            data1 = "{stdout}".format(stdout=json.dumps(result._result, indent=4))
            data2 = "{host}|{stdout}".format(host=result._host.get_name().decode('utf-8'),
                                             stdout=json.dumps(result._result, indent=4))

        DsRedis.OpsAnsibleModel.lpush(self.redisKey, data)
        config = Global_Config.objects.get(id=1)
        if config.tf_model_log == 1:
            if self.logId: AnsibleSaveResult.Model.insert(self.logId, data2, result._host.get_name().decode('utf-8'), 0,
                                                          data1)
        else:
            if self.logId: AnsibleSaveResult.Model.insert(self.logId, data, result._host.get_name().decode('utf-8'), 0,
                                                          data1)
    ...
    def v2_runner_on_failed(self, result, *args, **kwargs):
        ...
        if result._result.has_key('rc') and result._result.has_key('stdout'):
            if result._result.get('stdout').strip() == '' and result._result.has_key('stderr'):
                data2 = "{host} | FAILED | rc={rc} >> \n{stdout}".format(host=result._host.get_name().decode('utf-8'),
                                                                         rc=result._result.get('rc'),
                                                                         stdout=result._result.get('stderr'))
                data = "{host} | FAILED!".format(host=result._host.get_name().decode('utf-8'))
                data1 = "{stdout}".format(stdout=result._result.get('stderr'))
            else:
                data2 = "{host} | FAILED | rc={rc} >> \n{stdout}".format(host=result._host.get_name().decode('utf-8'),
                                                                         rc=result._result.get('rc'),
            ...
            data2 = "{host} | FAILED! => {stdout}".format(host=result._host.get_name().decode('utf-8'),
                                                          stdout=json.dumps(result._result, indent=4))
            data = "{host} | FAILED!".format(host=result._host.get_name().decode('utf-8'))
            data1 = "{stdout}".format(stdout=json.dumps(result._result, indent=4))
        DsRedis.OpsAnsibleModel.lpush(self.redisKey, data)
        if self.logId: AnsibleSaveResult.Model.insert(self.logId, data2, result._host.get_name().decode('utf-8'), 1,
                                                      data1)
...
class PlayBookResultsCollectorToSave(CallbackBase):
    ...
    def v2_runner_on_ok(self, result, *args, **kwargs):
        ...
        if result._task.action in ('include', 'include_role'):
            ...
            if delegated_vars:
                msg = "changed: [%s -> %s]" % (result._host.get_name().decode('utf-8'), delegated_vars['ansible_host'])
            else:
                msg = "changed: [%s]" % result._host.get_name().decode('utf-8')
        else:
            if delegated_vars:
                # msg = "ok: [%s -> %s]=> {%s}" % (result._host.get_name().decode('utf-8'), delegated_vars['ansible_host'],json.dumps(result._result))
                msg = "ok: [%s -> %s]" % (result._host.get_name().decode('utf-8'), delegated_vars['ansible_host'])
            else:
                if result._result.get('msg', False):
                    msg = "ok: [%s]=> {msg:%s}" % (
                        result._host.get_name().decode('utf-8'), result._result.get('msg', None))
                else:
                    msg = "ok: [%s]" % result._host.get_name().decode('utf-8')
    ...
    def v2_runner_on_failed(self, result, *args, **kwargs):
        ...
        if delegated_vars:
            msg = "fatal: [{host} -> {delegated_vars}]: FAILED! => {msg}".format(
                host=result._host.get_name().decode('utf-8'),
                delegated_vars=delegated_vars[
                    'ansible_host'],
                msg=json.dumps(result._result))
        else:
            msg = "fatal: [{host}]: FAILED! => {msg}".format(host=result._host.get_name().decode('utf-8'),
                                                             msg=json.dumps(result._result))
        DsRedis.OpsAnsiblePlayBook.lpush(self.redisKey, msg)
        if self.logId: AnsibleSaveResult.PlayBook.insert(self.logId, msg)
    ...
    def v2_runner_on_unreachable(self, result):
        ...
        msg = "fatal: [{host}]: UNREACHABLE! => {msg}\n".format(host=result._host.get_name().decode('utf-8'),
                                                                msg=json.dumps(result._result))
        DsRedis.OpsAnsiblePlayBook.lpush(self.redisKey, msg)
        if self.logId: AnsibleSaveResult.PlayBook.insert(self.logId, msg)
    ...
    def v2_playbook_on_play_start(self, play):
        name = play.get_name().strip().decode('unicode-escape').encode('utf-8')
        if not name:
            msg = "PLAY"
        else:
            msg = "PLAY [%s]" % name
    ...
    def v2_playbook_on_stats(self, stats):
        ...
        for h in hosts:
            ...
            DsRedis.OpsAnsiblePlayBook.lpush(self.redisKey, msg)
            if (int(t['unreachable']) > 0 or int(t['failures']) > 0):
                if self.logId: AnsibleSaveResult.PlayBook.insert(self.logId, msg, 1)
            else:
                if self.logId: AnsibleSaveResult.PlayBook.insert(self.logId, msg, 0)
...
class PlayBookResultsCollectorToSaveDB(CallbackBase):
    CALLBACK_VERSION = 2.0

    def __init__(self, logId, *args, **kwargs):
        super(PlayBookResultsCollectorToSaveDB, self).__init__(*args, **kwargs)
        self.task_ok = {}
        self.task_skipped = {}
        self.task_failed = {}
        self.task_status = {}
...
class ANSRunner(object):
    ...
    def __init__(self, resource, redisKey=None, logId=None, forks=5, check=False, isSudo=0, *args, **kwargs):
        self.isSudo = isSudo
        self.forks = forks
        self.check = check
        self.resource = resource
    ...
    def __initializeData(self):
        ...
        if self.isSudo == 1:
            is_become = True
            is_become_method = 'sudo'
        else:
            is_become = None
            is_become_method = None

        self.options = Options(connection='smart', module_path=None, forks=int(self.forks), timeout=10,
                               remote_user='root', ask_pass=False, private_key_file=None, ssh_common_args=None,
                               ssh_extra_args=None,
                               sftp_extra_args=None, scp_extra_args=None, become=is_become, become_method=is_become_method,
                               become_user='root', ask_value_pass=False, verbosity=None, check=self.check,
                               listhosts=False,
    ...
    def run_playbook(self, host_list, playbook_path, extra_vars=None):
        ...
        try:
            if self.redisKey:
                ...
                self.callback = PlayBookResultsCollectorToSaveDB(self.logId)
            ...
            DsRedis.OpsAnsibleModel.lpush(self.redisKey, data=err)
            # if self.logId: AnsibleSaveResult.Model.insert(self.logId, err)
            if self.logId: AnsibleSaveResult.PlayBook.insert(self.logId, err)
            return False
...
class playbookforyaml():
    def __init__(self):
        self.path = "/etc/ansible/playbook/"
...
if __name__ == '__main__':
    ...
    resource = {
        "dynamic_host": {
            "hosts": [
                {"hostname": "10000号管家_岳阳1", "ip": "218.75.166.51", "port": "22", "username": "Wp9laOYyhkNt"},
    ...
    rbt.run_model(host_list=["10000号管家_岳阳1"], module_name='ping', module_args="")
