(function(editors, elFinder) {
	console.log('receiving', editors, elFinder);
			...
			load : function(textarea) {
				var self = this,
					...
					start = function() {
						...
						var hasRoles = editor.find('roles', {
							caseSensitive: true,
							wrap: true,
							wholeWord: true
						});

						var appearedLine = hasRoles !== undefined ? hasRoles.start.row : -1;

						try {
							var Range = ace.require('ace/range').Range;
							editor.session.addFold('...', new Range(0, 0, appearedLine - 1, Number.MAX_VALUE));
						}
						catch(err) {
    						window.wxc.xcConfirm('该Playbook文件未包含Roles.', window.wxc.xcConfirm.typeEnum.error);
						}

						editor.resize();

						// TextArea button and Setting button
						var functionButtons = $('<div class="ui-dialog-buttonset"/>').css('float', 'left')
						//.append(
						...
						//)
						.append(
							$('<button/>').html(self.fm.i18n('Playbook验证'))
							.button()
							.button()
							.on('click', function(){
						...
						$('<pre id="VRO" class="hide VRO"></pre>').css('text-align', 'left').insertBefore(functionButtons);
					};
