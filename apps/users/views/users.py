...
def user_center(request):
    if request.method == "GET": 
        ...
        return render_to_response('users/user_center2.html',{"user":request.user,"orderList":orderList},
                                  context_instance=RequestContext(request)) 
    if request.method == "POST":
        if request.POST.get('password') == request.POST.get('c_password'):
            try:
...
def user(request,uid):    
    if request.method == "GET":
        ...
        try:
            ...
            return HttpResponseRedirect('/users/user/{uid}/'.format(uid=uid))
        except Exception,e:
...
def group(request,gid):    
    if request.method == "GET":
        ...
        try:
            ...
            return render_to_response('group/group_info.html',{"user":request.user,"permList":permList,"group":group},context_instance=RequestContext(request))
        except Exception,e: 
        ...
        try:
            ...
            return HttpResponseRedirect('/users/group/{gid}/'.format(gid=gid))
        except Exception,e:
            return render_to_response('users/user_info.html',{"user":request.user,"errorInfo":"用户资料修改错误：%s" % str(e)},
                                              context_instance=RequestContext(request))         
